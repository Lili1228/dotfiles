...
case "$ID" in
	alpine)
		$_SUDO apk add feh gammastep htop mpv neovim py3-psutil py3-xlib qtile
		;;
	arch|artix)
		# wallpaper, night light, media player, editor, notifications, CPU usage, screen detection
    	$_SUDO pacman -Syu feh gammastep htop mpv neovim-qt python-dbus-next python-psutil python-xlib qtile
		;;
	debian|devuan|linuxmint|ubuntu)
		...
		;;
	fedora)
		$_SUDO dnf install feh gammastep-indicator htop mpv neovim-qt python3-cairocffi python3-dbus-next python3-pip python3-psutil python3-xlib
		$_SUDO pip3 install qtile
		;;
	*)
